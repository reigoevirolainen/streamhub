-- STREAMHUB V7 DATABASE
-- IMPORTANT: V7 website calls admin_add_streamer with EXACTLY 5 arguments.
...
-- Remove both the old 7-arg and old 5-arg versions.
...
BEGIN
  IF auth.uid() IS NULL THEN
    RAISE EXCEPTION 'Kasutaja ei ole sisse logitud';
  END IF;
  ...
  IF p_platform NOT IN ('Twitch','YouTube','Kick','TikTok') THEN
    RAISE EXCEPTION 'Lubatud platvormid: Twitch, YouTube, Kick, TikTok';
  END IF;

  INSERT INTO public.streamers
    (name, platform, channel_url, game, avatar_url, thumbnail_url, live_video_id, is_live, viewers, created_at, updated_at)
  VALUES
    (trim(p_name), p_platform, trim(p_channel_url),
     NULLIF(trim(COALESCE(p_game,'')), ''),
     NULLIF(trim(COALESCE(p_avatar_url,'')), ''),
     NULL, NULL, false, 0, now(), now())
...
-- Final check: must return parameter_count = 5.
